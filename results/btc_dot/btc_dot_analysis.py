...
# and assume a normal distribution (GBM) with the mean of zero and std of the KSM/BTC dot_btc over the past ~3 years
sim = Simulation(dot_btc, strategy="GBM")
sim.simulate(
    steps=1,
    maturity=config["analysis"]["thresholds"]["periods"]["safe_mint"],
    n_simulations=config["analysis"]["n_simulations"],
    initial_value=1,
    sigma=dot_btc.returns.std()[0],
    mu=0,
)
...
for key, threshold in thresholds.items():
    ...
    print(f"The {key} threshold based on the analytical VaR for a confidence level of {config['analysis']['alpha']*100}% of DOT/BTC over {config['analysis']['thresholds']['periods'][key]} is: {round(threshold['analytical_threshold'] *100,3)}%")
    print(f"The {key} threshold based on the historic VaR for a confidence level of {config['analysis']['alpha']*100}% of DOT/BTC over {config['analysis']['thresholds']['periods'][key]} is: {round(threshold['historical_threshold'] *100,3)}% \n")
