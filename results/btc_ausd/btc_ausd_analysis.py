...
print(
    f"aUSD/USD had an annualized std of {ausd_pair.returns.std()[0] *365**0.5}"
...
print(
    f"USD/BTC had an annualized std of {btc_usd.returns.std()[0] *365**0.5}"
)  # the std scaled by the square-root of time, 365 days to annulize it, **0.5 to square it
...
while end_date in btc_usd.returns.index:
    start_date = (
        ...
        in btc_usd.returns.index
        else btc_usd.returns.index[0]
    )
    print(
        f"The volatility of USD/BTC between {start_date} and {end_date} was {btc_usd.returns.loc[start_date:end_date,].values.std()*365**0.5*100}% \n"
    )
    end_date -= timedelta(config["analysis"]["historical_sample_period"])
...
for key, threshold in thresholds.items():
    ...
    hist_var = ausd_pair.prices.pct_change(
    ...
    print(
        f"The {key} threshold based on the analytical VaR for a confidence level of {config['analysis']['alpha']*100}% of aUSD/BTC over {config['analysis']['thresholds']['periods'][key]} days is: {round(threshold['analytical_threshold'] *100,3)}%"
    )
    print(
        f"The {key} threshold based on the historic VaR for a confidence level of {config['analysis']['alpha']*100}% of aUSD/BTC over {config['analysis']['thresholds']['periods'][key]} days is: {round(threshold['historical_threshold'] *100,3)}% \n"
    )
